...
void socket_util() {
    ...
    server_address = getAddress("127.0.0.1", 8080); // client address
    int client_addrlen = sizeof(server_address);


    char message[100];
    cout << "Enter message: ";
    cin.getline(message, 100);
    sendto(sfd, message, sizeof(message), 0, (struct sockaddr*)&server_address, (socklen_t)client_addrlen);
