...
int getPassiveRawSocket(int PORT) {
    int rsfd;
...
void sendNormalMessageToRawSocket(int rsfd, string ip, string message, int PORT = 8080) {
    ...
    sendto(rsfd, message.c_str(), message.size(),
        0, (const struct sockaddr*)&server_addr, sizeof(server_addr));

    cout << "Message send to raw socket\n";
}